...
#include <lin_model/inference_engine_tvm_config.hpp>
...
using model_zoo::inf_test::engine_load::linear_model::config;
...
public:
  ...
  : network_input_a_width(config.network_inputs[0].node_shape[0]),
    ...
    network_input_x_width(config.network_inputs[1].node_shape[0]),
    network_input_x_height(config.network_inputs[1].node_shape[1]),
    network_input_b_width(config.network_inputs[2].node_shape[0]),
    network_input_b_height(config.network_inputs[2].node_shape[1]),
    network_input_datatype_bytes(config.network_inputs[0].tvm_dtype_bits / 8)
  {
    ...
    std::vector<int64_t> shape_x{network_input_x_width, network_input_x_height};
    tvm_utility::pipeline::TVMArrayContainer x{
      shape_x,
      config.network_inputs[1].tvm_dtype_code,
      config.network_inputs[1].tvm_dtype_bits,
      config.network_inputs[1].tvm_dtype_lanes,
      config.tvm_device_type,
      config.tvm_device_id};
    std::vector<int64_t> shape_b{network_input_b_width, network_input_b_height};
    tvm_utility::pipeline::TVMArrayContainer b{
      shape_b,
      config.network_inputs[2].tvm_dtype_code,
      config.network_inputs[2].tvm_dtype_bits,
      config.network_inputs[2].tvm_dtype_lanes,
      config.tvm_device_type,
      config.tvm_device_id};
  ...
  tvm_utility::pipeline::TVMArrayContainerVector schedule(const std::vector<float> & input)
  {
    ...
    cv::Mat x_input = cv::Mat(2, 2, CV_32F, &input_mat);
    cv::Mat b_input = cv::Mat(2, 2, CV_32F, &input_mat);
    ...
    TVMArrayCopyFromBytes(
      x.getArray(), x_input.data,
      network_input_x_width * network_input_x_height *
        network_input_datatype_bytes);
    TVMArrayCopyFromBytes(
      b.getArray(), b_input.data,
      network_input_b_width * network_input_b_height *
        network_input_datatype_bytes);

    std::cerr << "push to output" << std::endl;
...
private:
  ...
  int64_t network_input_x_width;
  int64_t network_input_x_height;
  int64_t network_input_b_width;
  int64_t network_input_b_height;
  int64_t network_input_datatype_bytes;
  tvm_utility::pipeline::TVMArrayContainer a;
  tvm_utility::pipeline::TVMArrayContainer x;
  tvm_utility::pipeline::TVMArrayContainer b;
  std::vector<tvm_utility::pipeline::TVMArrayContainer> output;
};
...
public:
  ...
  std::vector<float> schedule(const tvm_utility::pipeline::TVMArrayContainerVector & input)
  {
    ...
    std::vector<float> infer(
      network_output_width * network_output_height, 0);
    TVMArrayCopyToBytes(
